...
CREATE POLICY "Allow read access to authenticated users" ON users FOR
SELECT USING (auth.role() = 'authenticated');
-- Allow users to update their own last_active and limited fields
CREATE POLICY "Allow users to update own record" ON users FOR
...
VALUES (
    ...
    -- Default role
    'Pending',
    -- Default status, needs admin approval
    COALESCE(
