  ...
  return (
    <div className="p-6 rounded-3xl bg-white dark:bg-surface-dark border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group duration-300">
      <div className="flex justify-between items-start mb-6">
        <div
          className={`w-12 h-12 rounded-[18px] ${stat.color} bg-opacity-10 dark:bg-opacity-20 flex items-center justify-center text-opacity-100 border border-current border-opacity-20`}
        >
        ...
        <div className="flex flex-col items-end gap-1.5">
          {stat.change && (
            <span
              className={`flex items-center text-[10px] font-black uppercase tracking-widest ${stat.isPositive ? "text-emerald-500 bg-emerald-50 dark:bg-emerald-900/30" : "text-rose-500 bg-rose-50 dark:bg-rose-900/30"} px-2.5 py-1 rounded-full border border-current border-opacity-10`}
            >
              {stat.change}
      ...
      <p className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400 dark:text-slate-500 mb-2 leading-none">
        {stat.label}
      </p>
